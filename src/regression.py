...
def train_epoch(model, train_loader, loss, optimizer, scheduler, num_samples):
    ...
    with tqdm(total=num_samples) as pbar:
        for batch in iter(train_loader):
            ...
            top_preds = (model.forward(batch) + 1).squeeze(1)
            labels = batch['labels'].float()

            train_loss = loss(top_preds, labels)
            accum_loss += train_loss
            accum_rmse += train_loss.item()**(1/2)
            train_loss.backward()
    ...
    return {
        ...
        'average_rmse': accum_rmse / batch_count
    }
    ...
    with torch.no_grad():
        ...
        with tqdm(total=num_samples) as pbar:
            for batch in iter(valid_loader):
                ...
                top_preds = (model.forward(batch) + 1).squeeze(1)
                top_preds = torch.round(top_preds).float()
                labels = batch['labels'].float()

                valid_loss = loss(top_preds, labels)
                accum_loss += valid_loss
                accum_rmse += valid_loss.item()**(1/2)
...
def main():
    ...
    if mode == "test":
        ...
        model = regression_model(bert_model).to(device)
        try:
